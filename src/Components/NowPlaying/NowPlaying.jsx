  ...
  useEffect(() => {
    const getNowPlayingMovies = async () => {
      try {
        ...
        localStorage.setItem('nowPlayingMovies', JSON.stringify(response.data.results));
      ...
      <Link to={`/details`}>
        <div className='flex flex-col sm:flex-row border-b-4 border-red-700 border-solid'>
          ...
          <div className="w-full sm:w-4/5 px-2">
            <Slider {...settings}>
              {nowPlaying?.map((nowPlayingMovie, index) => (
                <Link  to={`/details/${nowPlayingMovie.id}`}>
                  <div key={index} className='p-2 rounded-lg'>
                    ...
                    />
                    <p className='mt-2 text-white text-center'>{nowPlayingMovie.title}</p>
                  </div></Link>
              ))}
            </Slider>
          </div>
          ...
          <div className='flex flex-col w-full sm:w-1/5 mb-4 sm:mb-0'>
            <p className={`${style.top} text-white`}>NowPlaying Movies</p>
